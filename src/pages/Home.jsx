...
const Partners = () => {
  ...
  return (
    <section className="border-b border-border-primary bg-bg-primary py-24">
      <div className="mx-auto max-w-7xl px-6">
        ...
        <motion.div
          variants={stagger} initial="hidden" whileInView="visible" viewport={{ once: true }}
          className="grid grid-cols-2 items-center gap-x-6 gap-y-10 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6"
        >
          {partners.items.map((p, i) => (
            <motion.div key={i} variants={fadeInUp} className="flex items-center justify-center">
              <img
                src={p.src}
                alt={p.name}
                loading="lazy"
                title={p.name}
                className="max-h-12 w-auto object-contain opacity-60 grayscale transition-all duration-300 hover:opacity-100 hover:grayscale-0 dark:opacity-50 dark:invert dark:hover:opacity-100 dark:hover:invert-0"
              />
            </motion.div>
          ))}
        </motion.div>
